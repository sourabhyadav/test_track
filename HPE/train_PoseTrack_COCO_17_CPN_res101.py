...
def create_refine_net(blocks, is_training, trainable=True):
    ...
    for i, block in enumerate(blocks):
        ...
        with slim.arg_scope(resnet_arg_scope(bn_is_training=is_training)):
            for j in range(i):
                mid_fm = bottleneck(mid_fm, 256, 128, stride=1,
                                    scope='res{}/refine_conv{}'.format(2 + i, j))  # no projection
...
class Network(ModelDesc):
    ...
    def make_network(self, is_train):
        ...
        if is_train:
            ...
            refine_loss = tf.reduce_mean(tf.square(refine_out - label7), (1, 2)) * tf.to_float(
