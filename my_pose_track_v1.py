...
def light_track(pose_estimator,
    ...
    global video_name
    '''
    gt_info: type list, 包含image_item,
    '''
        ...
        if labeled and (img_id - first_img_id) % 5 == 0:
            ...
            if img_id==first_img_id:
                ...
                for det_id in range(num_dets):
                    ...
                    scores = np.zeros_like((num_dets, len(keypoints_list_prev_frame)))
                    ...
                    for prev_det_id in keypoints_list_prev_frame:
                ...
                bbox_dets_list = bipartite_graph_matching(bbox_dets_list, keypoints_list_prev_frame)
                # # 先用距离相似度来判断
